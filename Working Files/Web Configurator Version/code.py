...
ONBOARD BUTTONS / EXTERNAL SWITCHES (mode selection):
  D1 or A2 (select switch) → Enter Single-Switch Scanning Mode
  D2 or A1 (nav switch)    → Enter Two-Switch Mode
...
def draw_menu_screen():
    ...
    # Left half — D1 or A2 (select switch) → Single-Switch mode
    splash.append(label.Label(
        terminalio.FONT, text="D1/A2", color=WHITE, scale=2,
        anchor_point=(0.5, 0.5),
        anchored_position=(SCREEN_WIDTH // 4, SCREEN_HEIGHT // 2 - 20)
    ))
    splash.append(label.Label(
    ...
    splash.append(label.Label(
        ...
        anchor_point=(0.5, 0.5),
        anchored_position=(SCREEN_WIDTH // 4, SCREEN_HEIGHT // 2 + 28)
    ))

    # Right half — D2 or A1 (nav switch) → Two-Switch mode
    splash.append(label.Label(
        terminalio.FONT, text="D2/A1", color=WHITE, scale=2,
        anchor_point=(0.5, 0.5),
        anchored_position=(3 * SCREEN_WIDTH // 4, SCREEN_HEIGHT // 2 - 20)
    ))
    splash.append(label.Label(
    ...
    splash.append(label.Label(
        ...
        anchor_point=(0.5, 0.5),
        anchored_position=(3 * SCREEN_WIDTH // 4, SCREEN_HEIGHT // 2 + 28)
    ))
...
print("D1 or A2 (select switch) = Single-Switch Scanning Mode")
print("D2 or A1 (nav switch)    = Two-Switch Mode")
...
while True:
    try:
        draw_menu_screen()
        print("MODE SELECT: Press D1 or A2 (select) for single-switch, D2 or A1 (nav) for two-switch")

        # Drain any buttons/switches still held from a previous mode before listening
        while d1_pressed() or d2_pressed() or select_pressed() or nav_pressed():
            time.sleep(0.01)

        # Wait for a fresh press — D1 or A2 (select switch) → single-switch mode
        #                          D2 or A1 (nav switch)    → two-switch mode
        while True:
            if d1_pressed() or select_pressed():
                while d1_pressed() or select_pressed():
                    time.sleep(0.01)
                run_single_switch_mode()
                break
            if d2_pressed() or nav_pressed():
                while d2_pressed() or nav_pressed():
                    time.sleep(0.01)
                run_two_switch_mode()
                break
            time.sleep(0.01)
